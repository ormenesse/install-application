...
def paymentCapacityPriceTable(request, gyraFeesPath='./install_csv/gyra_fees.csv'):
    ...
    try:
        ...
        if partner not in list(gyra_fees['type'].unique()):
            
            partner = 'GYRA'

        if risk_group not in list(gyra_fees['riskGroup'][(gyra_fees['type'] == partner)].unique()):

            risk_group = gyra_fees['riskGroup'][(gyra_fees['type'] == partner)].min()
        ...
        for period in [3,6,9,12,18,24]: #[6,9,12,18,24]:
            ...
            interest_rate = np.float(gyra_fees['interest'][(gyra_fees['period'] == period) & (gyra_fees['type'] == partner) & (gyra_fees['riskGroup'] == risk_group)].max())
            ...
            if fees == True:
                ...
                for col in list(gyra_fees.columns):
                    ...
                    if 'Fee' in col:
                        ...
                        try:
                            fee = gyra_fees[col][(gyra_fees['period'] == period) & (gyra_fees['type'] == partner)].max()
                        except:
                            fee = gyra_fees[col][(gyra_fees['type'] == partner)].max()
